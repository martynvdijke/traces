...
func TestEinkConfigHandlers(t *testing.T) {
	gin.SetMode(gin.TestMode)

	origDB := db
	origEinkEnabled := einkEnabled
	defer func() {
		db = origDB
		einkEnabled = origEinkEnabled
	}()
	...
	db.Exec(`INSERT OR IGNORE INTO site_settings (id, eink_enabled) VALUES (1, 0)`)
	einkEnabled = false

	r := gin.New()
	r.GET("/api/eink/config", getEinkConfig)
	r.POST("/api/eink/config", saveEinkConfig)
	r.GET("/api/config", getPublicConfig)

	t.Run("get_config_default", func(t *testing.T) {
		req := httptest.NewRequest("GET", "/api/eink/config", nil)
		w := httptest.NewRecorder()
		r.ServeHTTP(w, req)
		...
		if cfg.EinkEnabled {
			t.Error("EinkEnabled should be false by default")
		}
	})

	t.Run("save_and_read_config", func(t *testing.T) {
		cfg := models.SiteConfig{EinkEnabled: true}
		body, _ := json.Marshal(cfg)

		req := httptest.NewRequest("POST", "/api/eink/config", bytes.NewReader(body))
		req.Header.Set("Content-Type", "application/json")
		w := httptest.NewRecorder()
		r.ServeHTTP(w, req)
		...
		if !einkEnabled {
			t.Error("einkEnabled global should be true after save")
		}

		// Read back via GET
		req2 := httptest.NewRequest("GET", "/api/eink/config", nil)
		w2 := httptest.NewRecorder()
		r.ServeHTTP(w2, req2)
		...
		var readCfg models.SiteConfig
		json.Unmarshal(w2.Body.Bytes(), &readCfg)
		if !readCfg.EinkEnabled {
			t.Error("EinkEnabled should be true after save")
		}
	})

	t.Run("toggle_off", func(t *testing.T) {
		cfg := models.SiteConfig{EinkEnabled: false}
		body, _ := json.Marshal(cfg)
		...
		if einkEnabled {
			t.Error("einkEnabled should be false after saving disabled config")
		}
	})

	t.Run("public_config_returns_eink", func(t *testing.T) {
		einkEnabled = true
		...
		einkVal, ok := cfg["eink_enabled"]
		if !ok {
			t.Fatal("public config should include eink_enabled")
		}
		if einkVal != true {
			t.Errorf("eink_enabled = %v, want true", einkVal)
		}
	})
}
